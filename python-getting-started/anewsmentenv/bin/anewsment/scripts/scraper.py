...
# function to get random user agent when making url requests
def get_user_agent():
    '''
...
#############################
#                           #
#   La Jornada Webscraper   #
#                           #
#############################

# main_url = "http://www.jornada.unam.mx/ultimas"
years07_09 = ['2007', '2008'] # for first first two months of 2009, we used this list, but the website change following february 2009
years10_17 = ['2009', '2010', '2011', '2012', '2013', '2014', '2015', '2016','2017']
visit = ['politica', 'economia', 'estados', 'sociedad', 'mundo', 'ciencias',
        'cultura']
# for the entire data scrape we will get 2008,04,01 to 2013,04,01
...
def get_sections(main_url, tag_type, class_type):
    ...
    #La Jornada section format between 2017
    tag_list = soup.find_all(tag_type, class_= class_type)
    if tag_list:
        rel_links_menu = []
        for t in tag_list: 
...
#############################
#                           #
#  Tribune Archive Scraper  #
#                           #
#############################

def write_csv_tr(dictionary, filename):
    '''
    Writes csv file for a given info dictionary
    Inputs :
            Info dictionary for a given day
    ...
    '''
    with open(filename, 'w') as csv_file:
        fieldnames = ['article','pub_date','source', 'nltk_score', 'nltk_score_title']
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames)#delimiter='|')
        writer.writeheader()
        for value in dictionary.values():
            writer.writerow(value)
...
def get_articles_c_tribune(complement):
    ...
    if tag_list:
        ...
        write_csv_tr(articles, 'chicago_tribune_'+ re.sub("/", "_", complement) +'.csv')
...
# we tried to make an ifmain to run this file, but ran into issues with the url requsts timing out
# instead, we ran the download functions in ipython3 to download the relevant csv files
...
#     download_tribune(start_date, end_date)
